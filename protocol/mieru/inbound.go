...
type Inbound struct {
	...
	userNames []string
	userconns sync.Map
	mux       *mieruprotocol.Mux // Cached mux reference to avoid reflection overhead
...
func (h *Inbound) Start(stage adapter.StartStage) error {
	...
	// Initialize mux reference once to avoid repeated reflection operations
	h.mux = h.getMuxFromServer()
	if h.mux == nil {
		// This should not happen in normal cases, log and continue
		// User updates will fail if mux is not available
		h.logger.Warn("failed to get mux from mieru server, dynamic user updates may not work")
	}
...
// getMuxFromServer uses reflection to access the private mux field from the mieru server.
// This is called once during Start() and the result is cached to avoid repeated reflection overhead.
func (h *Inbound) getMuxFromServer() *mieruprotocol.Mux {
	serverValue := reflect.ValueOf(h.server)
	if serverValue.Kind() == reflect.Interface {
	...
	serverValue = serverValue.Elem()
	if serverValue.Kind() != reflect.Struct {
		return nil
	}

	muxField := serverValue.FieldByName("mux")
	if !muxField.IsValid() {
		return nil
	}

	// Use unsafe to access the private field
	// Get the pointer to the mux field
	muxPtr := unsafe.Pointer(muxField.UnsafeAddr())
	// Convert to *mieruprotocol.Mux
	mux := *(**mieruprotocol.Mux)(muxPtr)
...
func buildMieruServerConfig(_ context.Context, options option.MieruInboundOptions) (*mieruserver.ServerConfig, []string, error) {
	...
	port := int32(options.ListenOptions.ListenPort)
